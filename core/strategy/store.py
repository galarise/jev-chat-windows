...
# 插件在 core/strategy/ 下，多退一层到项目根；data/ 与主程序一致
...
def append_messages(chat_key: str, lines: list) -> int:
    ...
    for i, item in enumerate(lines):
        ...
        if not text or not sender:
            continue
        # ts 粒度 0.1s：用整数格（time*10 取整）而不是 round(ts,1)——后者在 x.x5 附近
        # 有浮点表示 + 舍入双重抖动，同秒两批可能落到不同格，幂等就破了
        rows.append((str(chat_key), str(sender), str(text), int(ts * 10) / 10 + i * 0.001))
    if not rows:
        return 0
    with _connect() as conn:
